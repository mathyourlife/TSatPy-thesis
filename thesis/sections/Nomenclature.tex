...
\nomenclature{$\bs{q}$}{quaternion where $\bs{q} = q_1\bs{i}+q_2\bs{j}+q_3\bs{k}+q_0$}
\nomenclature{$\bs{q_k}$}{quaternion state at discrete step $k$}
\nomenclature{$\bs{q_r}$}{rotational quaternion}
\nomenclature{$\bs{q_I}$}{identity quaternion where $\bs{q_I} = 0\bs{i}+0\bs{j}+0\bs{k}+1$}
\nomenclature{$\bs{\Omega}$}{skew symmetric matrix}
