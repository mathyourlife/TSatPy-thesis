function studentdave_kalmanfilter_code()
  ...
  Q = [-20 5]' %initized state--it has two components: [position; velocity] of the Quail
  Q_estimate = [0 0]'; %x_estimate of initial location estimation of where the Quail is (what we are updating)
  ...
  for t = 0 : dt: duration
    ...
    Q_adjustment = K * (Q_loc_meas(inc) - C * Q_estimate);
    Q_estimate = Q_estimate + Q_adjustment;
    ...
    %disp(sprintf('Kalman Adjustment: %0.6f',sum(abs(Q_adjustment'))))
    %disp(sprintf('State Error: %0.6f',abs(y-Q_estimate(1))))
  ...
  %plot(0:dt:t, filter([1 1 1 1]/4,1,Q_loc_meas), '-m.')
  ...
  for t = 1:length(Q_loc)
    ...
    disp(sprintf('%0.6f %0.6f +=\t%0.6f %0.6f',Q_estimate, K * (Q_loc_meas(t) - C * Q_estimate)))
    % update covariance estimation.
    P = (eye(2) - K * C) * P;
    %Store for plotting
    Q_loc_estimate = [Q_loc_estimate; Q_estimate(1)];
