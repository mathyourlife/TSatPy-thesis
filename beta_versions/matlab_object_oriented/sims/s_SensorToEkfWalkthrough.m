...
function plot_css_voltages(duration)
  ...
  while start_sim + duration > t.now()
    ...
    for css_num = 1:6
      item = struct; item.name = sprintf('css_%d', css_num); item.type = 'plot';
      data = struct; data.x = h.logs.css(:,1)-cur_time;
      data.y = h.logs.css(:,css_num + 1); item.data = data;
...
function plot_css_angle(duration)
  ...
  while start_sim + duration > t.now()
    ...
    item = struct; item.name = 'css_angle'; item.type = 'plot';
    data = struct; data.x = h.logs.css_theta(:, 1) - cur_time;
    data.y = h.logs.css_theta(:, 2); item.data = data;
    args = struct; args.action = 'updateseries'; args.graph = graph_name; args.item = item;
    graphManager(args);
...
function plot_css_mag_voltages(duration)
  ...
  while start_sim + duration > t.now()
    ...
    for mag_num = 1:3
      ...
      data = struct; data.x = h.logs.mag(:,1)-cur_time;
      data.y = h.logs.mag(:,mag_num + 1); item.data = data;
      args = struct; args.action = 'updateseries'; args.graph = graph_name; args.item = item;
      graphManager(args);
    end
