...
disp(['Testing the connection between Desired Moment - Thruster Activation - ' ...
  'Effective Moment - Plant Propagation - 3D Display'])
...
title(['Euler Moment Equations and Quaternion Dynamics with Random Thruster ' ...
  'Moment Requests'])
...
info = text(0.5,0.5,0.5,sprintf(['$$RequestedMoment = \\left( {\\matrix{ %0.2f ' ...
  '\\cr %0.2f \\cr %0.2f } } \\right)$$'],desired_moment(1),desired_moment(2), ...
  desired_moment(3)),'interpreter','latex','VerticalAlignment','bottom','FontSize',12);
for i=0:ceil(run_time/pause_time)
  ...
  if last_thrust + thruster_pulse_duration < t.now()
    ...
    set(info,'String',sprintf(['$$RequestedMoment = \\left( {\\matrix{ %0.2f ' ...
      '\\cr %0.2f \\cr %0.2f } } \\right)$$'],desired_moment(1),desired_moment(2), ...
      desired_moment(3)));
  end
