...
label = struct;
label.lh = text(0,0,'Testing new','Parent',graphs.sim_plot_with_latex.obj.plot_id, ...
...
while end_time > t.now()
  ...
  data = struct; data.x = ts_1_hist.logs.ts_1(:,1)-cur_time;
  data.y = ts_1_hist.logs.ts_1(:,2); item.data = data;
  args = struct; args.action = 'updateseries';
  args.graph = 'sim_plot_with_latex'; args.item = item;
  graphManager(args);

  item = struct; item.name = 'test_series_2'; item.type = 'plot';
  data = struct; data.x = ts_1_hist.logs.ts_2(:,1)-cur_time;
  data.y = ts_1_hist.logs.ts_2(:,2); item.data = data;
  args = struct; args.action = 'updateseries';
  args.graph = 'sim_plot_with_latex'; args.item = item;
  graphManager(args);

  set(graphs.sim_plot_with_latex.obj.labels.cos.lh,'Position',[0 cos(cur_time/2) 0]);
  set(graphs.sim_plot_with_latex.obj.labels.cos.lh,'String', ...
    sprintf('$$cos\\left(\\frac{t}{2}\\right) = %0.4f$$',cos(cur_time/2)));
  set(graphs.sim_plot_with_latex.obj.labels.sin.lh,'Position',[0 sin(cur_time/2) 0]);
  set(graphs.sim_plot_with_latex.obj.labels.sin.lh,'String', ...
    sprintf('$$sin\\left(\\frac{t}{2}\\right) = %0.4f$$',sin(cur_time/2)));
  if (record_it)
    mov(frame)=getframe(fh);
  end
end
if (record_it)
  movie2avi(mov,'realtime_plotting_latex_updates.avi', 'COMPRESSION', ...
    'Cinepak', 'FPS', floor(1/pause_time));
  clear mov;
end
