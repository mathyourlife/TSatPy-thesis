function s_KalmanFilterDemo()
  ...
  args.item.style = '--b'; args.item.LineWidth = 2;
  ...
  w = @(dt)[Q_std  * [(dt^2/2); dt]];    % Create process noise matrix function
  ...
  while t.now() < start_time + 3 || mean(abs(h.logs.est(:,2) - h.logs.truth(:,2))) > 1.5
    ...
    msg = sprintf(notes,R_std^2,Q_std^2,t.now()-start_time,dt,x_truth(1),y, ...
      x_est(1), error);
