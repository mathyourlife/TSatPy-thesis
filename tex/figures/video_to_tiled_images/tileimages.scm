...
;; Added file name sorting to properly order the images
;; -- Dan Couture
(define (name-sort lis)
  (define (insert name sorted)
    (if (null? sorted)
      (list name)
      (if (string<? name (car sorted))
        (cons name sorted)
        (cons (car sorted) (insert name (cdr sorted))) )))
  (if (null? lis)
        '()
        (insert (car lis) (name-sort (cdr lis))) ))
...
(define (batch-tileimages fileout n m padding gimp-fill-type roundedcornerradius featherselectionradius drawbox drawgrid gridlinewidth dpi jpeg-quality file-glob-pattern)
  (let* ((filelist (name-sort (cadr (file-glob file-glob-pattern 1))))
