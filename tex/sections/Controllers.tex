...
The initial convergence of to the spin stabilized state is rapid as in previous test configurations.  In this case, however, the controller is not able to fully remove the attitude error.  Additional gain tuning or the inclusion of advanced visualizations such as the tPlot discussed in Section \ref{sec:ObjectOrientedNSSControlSystem} is needed to help diagnose the source of the error.
\begin{figure}[H]
