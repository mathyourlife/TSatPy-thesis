...
\TODO{From the results above it seems that the Ford Focus antenna could be used to model the ADP on TableSat as long as there is a mass adjustment on the antenna itself raising the frequency of the antenna. It would seem that the Ford Focus antenna would most likely be able to handle a wider range of frequencies because initially it has the lowest frequency. To model the 0.1 Hz ADP with the Ford antenna 3.646 Kg would have to be added to the antenna but for 0.3 Hz only 0.368 Kg would have to be added. Thus scaling TableSat up a factor of three while using the Ford antenna seems feasible. \cite{laakso}}

\begin{figure}[ht]
  \centerline{\psfig{file=figures/sdp_boom_droop.eps,height=4in}}
...
\begin{figure}[ht]
  \centerline{\psfig{file=figures/css_mounted.eps,height=4in}}
