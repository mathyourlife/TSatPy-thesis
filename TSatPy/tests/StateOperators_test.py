...
from TSatPy import State, StateOperators
import numpy as np
...
class TestQuaternionSaturation(unittest.TestCase):
    ...
    def test_init(self):
        s = StateOperators.QuaternionSaturation(4)
    ...
    def test_limit(self):
        s = StateOperators.QuaternionSaturation(1.1)
...
class TestBodyRateSaturation(unittest.TestCase):
    ...
    def test_init(self):
        s = StateOperators.BodyRateSaturation(0.3)
        self.assertTrue(np.all(s.rho == np.mat([0.3,0.3,0.3]).T))

    def test_str(self):
        s = StateOperators.BodyRateSaturation(2)
        self.assertEquals('<BodyRateSaturation <rho 2.0>>', str(s))

    def test_limit(self):
        s = StateOperators.BodyRateSaturation(1.3)
